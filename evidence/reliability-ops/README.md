...
| Grafana panels exist as committed exports, not just a spec | Two dashboards: `devops-g1-capacity-dashboard` (RPS + target-5xx from the ALB) and `devops-g1-slo-dashboard` (#41 — 18 panels: uptime 5m/1h/28d, error budget remaining over 28d, fast/slow burn matching the alarm thresholds, latency/CPU/queue-age), both bound to live data | [`grafana-dashboard-export.md`](grafana-dashboard-export.md), [`grafana-slo-dashboard.md`](grafana-slo-dashboard.md) |
...
| G4 2.9 (resource saturation) executed for real, not just asserted, with a real Grafana correlation | 15-min k6 soak against the deployed target: checks 100%, `http_req_failed` 0%, p95 61ms, ~40 rps sustained at 15 VUs -- under the live edge throttle. Grafana screenshot confirms latency/CPU/memory rise and fall exactly with the run, queue age stays flat throughout | [`g4-soak-drill.md`](g4-soak-drill.md) · [`screenshot`](g4-soak-grafana-correlation.png) |
| G3's k6 envelope, baseline half -- stepped ramp to 100 VUs against the deployed target | Thresholds breach by design once load crosses the live 50 rps API Gateway throttle (`checks` 32%, `http_req_failed` 66%), while latency for requests that DO get through stays flat (p95 61ms, no degradation) -- the signature of a gate rejecting requests, not POS buckling. Read as edge-limited, not POS's real ceiling; the exact per-step knee needs the raw per-request JSON, not pulled for this run | [`k6-baseline-run.md`](k6-baseline-run.md) |
| G3's k6 envelope, spike half -- sudden 10x (100 VUs) against the deployed target, all three scenarios now run against the real edge | Same edge-limited signature as baseline: loose thresholds breach (`checks` 21%, `http_req_failed` 79%) but successful-request latency stays flat (p95 103ms) and the run recovers cleanly once the burst ends | [`k6-spike-run.md`](k6-spike-run.md) |
| A real trace, captured end to end (OTLP -> ADOT -> X-Ray), not just wired | A real `POST /api/pos/tenants` produced a trace with the full inbound-HTTP + DB-query span chain (`pos` 201 -> pg-pool.connect -> pg.connect -> BEGIN -> two INSERTs -> COMMIT), screenshotted from the AWS X-Ray console after the Grafana-side X-Ray data source hit a real plugin bug (empty config UI) -- worked around, not ignored | [`g3-trace-capture.md`](g3-trace-capture.md) · [`screenshot`](g3-trace-capture.png) |
