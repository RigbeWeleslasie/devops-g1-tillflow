...
| Uptime, SLI, error-budget and burn-rate panels | **Authored, NOT yet imported or verified.** Importable JSON mirroring the alarms' own queries; becomes evidence only after it is imported, each panel is confirmed to render live data, and it is re-exported. What is unverified, and the sanity gate for trusting a green panel, are stated in the doc | [`grafana-slo-dashboard.md`](grafana-slo-dashboard.md) |
