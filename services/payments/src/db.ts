/**
 * DB access. Same shape and reasoning as services/pos/src/db.ts: business
 * logic takes a `Db` rather than importing a pool, so tests hand it a pg-mem
 * pool with the identical interface — real `pg`, not a hand-rolled subset.
 *
 * Every invariant in ADR 0006 is a transaction boundary:
 *   - I3: callback dedupe row + guarded status update + outbox row commit
 *     together or not at all.
 *   - I4: ledger row + payout row likewise.
 * `withTransaction` is the one place that boundary is drawn.
 */
import pg from 'pg';
