 ...
 * test body runs, and services/commission/src/metrics.ts builds its
 * instruments on first use. So the provider is always in place before the
 * first `add()`.
 *
 * A near-copy of services/payments/test/metricsHarness.ts. Kept per workspace
 * rather than shared, for the same reason `testDb.ts` is: the only home a
 * shared version could have is `services/_shared/ts`, which is Meron's, and a
 * test-only helper is not worth a cross-owner dependency. If a third service
 * needs it, that is the moment to move it.
 */
import {
