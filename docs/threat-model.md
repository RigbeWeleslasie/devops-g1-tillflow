...
| Daraja callback IP allow-list may be unavailable in sandbox | **Closed for STK at G5.** A success callback is no longer treated as evidence: before any `PENDING->PAID` transition Payments queries Daraja directly (`stkQuery`) over a channel the caller does not control, and pays only when the provider's own records agree. Daraja saying anything other than success is recorded, logged and metered as `contradicted` — the label that pages — and changes no state; a contradiction is deliberately **not** a hold, because freezing the charge would let anyone who can guess a reference deny service on that sale. An unreachable or still-processing query withholds the transition and leaves the charge to the reconciler (I5), never marking it PAID or FAILED on a check we could not run. The query is skipped entirely when no PENDING charge could be moved, so a forged reference costs one indexed SELECT and no outbound call. `CONFIRM_CALLBACKS=false` is an operator lever for a Daraja query-API outage, not a default. **Still open for B2C:** Daraja exposes no equivalent query for a disbursement in our adapter, so a B2C result callback rests on its reference being our own generated payout UUID (not guessable the way a CheckoutRequestID is) plus the amount cross-check. **Still open at the edge:** no IP allow-list — Meron, `infra/edge.tf`, if the sandbox publishes Safaricom's ranges. | Nebyat | STK done G5; B2C + allow-list carried |
