...
| Daraja callback IP allow-list may be unavailable in sandbox | Compensate with `CheckoutRequestID` match + amount cross-check. **As implemented (G2):** a callback is accepted only if its reference matches a charge/payout we issued AND its amount equals ours; an unverifiable amount is refused on both STK and B2C, and a mismatch puts the charge on hold for a human. There is **no** IP allow-list and **no** confirming `stkQuery` on success — a forged callback that guesses a live `CheckoutRequestID` and its exact amount would be accepted. Accepted for the capstone; the fix is an allow-list at the edge plus a confirming query before any PAID transition. | Nebyat | G5 |
